...
import { Bike, ShoppingBag, ArrowRight, Phone, UserCheck, Truck, Hash, MapPin, Store, Tag } from "lucide-react";
...
export default function OnboardingPage() {
  ...
  // merchant
  const [brandName, setBrandName] = useState(activeUser?.brand_name || "");
  const [category, setCategory] = useState(activeUser?.category || "Grocery & Fresh");
    ...
    const checkExistingProfile = async () => {
      try {
        ...
        if (currentRole === "merchant") {
          const snap = await getDoc(doc(db, "merchant_profiles", activeUser.email));
          if (snap.exists()) {
            ...
            if (data.onboarded || (data.brand_name && data.phone)) {
              const updatedUser = { ...activeUser, ...data, onboarded: true, role: "merchant" };
              setUser(updatedUser);
              await api.post("/onboarding/merchant", {
                phone: data.phone,
              ...
              toast.success("Merchant profile verified");
              navigate("/merchant", { replace: true });
            }
          }
        } else if (currentRole === "concierge" || currentRole === "rider") {
  ...
  const submitMerchant = async () => {
    ...
    if (!brandName.trim()) return toast.error("Please enter your Brand / Store Name");
    if (!address || !address.line1) return toast.error("Please set your Store Pickup Address");

    setSubmitting(true);
    try {
      const storeAddr = {
        label: address.label || "Store Location",
        flat: address.flat || "",
        ...
        searchAddress: [address.line1, address.area, address.city].filter(Boolean).join(", "),
      };
      ...
      const { data } = await api.post("/onboarding/merchant", payload);
      setUser(data);

      if (activeUser?.email) {
        try {
          await setDoc(doc(db, "merchant_profiles", activeUser.email), { ...payload, name: activeUser.name, email: activeUser.email }, { merge: true });
      ...
      toast.success(`Merchant Portal setup complete for ${brandName}!`);
      navigate("/merchant", { replace: true });
    } catch (e) {
      toast.error(e.response?.data?.detail || "Could not complete merchant onboarding");
    } finally {
      setSubmitting(false);
    }
  };
      ...
      >
        ...
        <div className="dz-card p-6 md:p-8 bg-white">
          <span className="dz-chip dz-chip-brand mb-4">
            ...
            {role === "merchant"
              ? "Merchant & Brand Portal Onboarding"
              : role === "concierge"
          ...
          <p className="text-neutral-700 font-medium mb-6">
            Please complete your {role === "merchant" ? "brand and store pickup" : role === "concierge" ? "delivery partner" : "delivery address and contact"} details below to get started.
          </p>
          ...
          {role === "merchant" ? (
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-5">
                <div>
                  <label className="dz-overline block mb-2">Store / Brand Name *</label>
                  <div className="relative">
                    <Store className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-neutral-500" strokeWidth={2.5} />
                    <input
                      data-testid="onboard-brand-name-input"
                      className="dz-input pl-10"
                      placeholder="e.g. Zepto Fresh, Zara Boutique, Apollo Pharmacy"
                      value={brandName}
                ...
                <div>
                  <label className="dz-overline block mb-2">Business Category *</label>
                  <div className="relative">
                    <Tag className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-neutral-500" strokeWidth={2.5} />
                    <select
                      data-testid="onboard-category-select"
                      className="dz-input pl-10 bg-white"
                      value={category}
              ...
              <div className="mb-5">
                <label className="dz-overline block mb-2">Store Pickup Address *</label>
                <p className="text-xs font-medium text-neutral-600 mb-3">
                  This address will be assigned as the pickup location for all customer order dispatches once accepted.
                </p>
                <AddressForm
                  testIdPrefix="merchant-store-addr"
                  onFormChange={(a) => setAddress(a)}
                  onSave={(a) => {
                    setAddress(a);
                    toast.success("Store pickup address recorded!");
                  }}
                  saving={submitting}
                />
              </div>
              ...
              <div className="pt-3 border-t-2 border-dashed border-black">
                <button
                  ...
                  disabled={submitting || !validPhone || !brandName.trim()}
                  className="dz-btn inline-flex items-center gap-2 !bg-[#22C55E] !text-black !py-3 !px-6 !text-sm font-bold border-2 border-black shadow-[3px_3px_0px_0px_#000]"
                >
                  {submitting ? "Activating Store…" : "Launch Merchant Portal"} <ArrowRight className="w-4 h-4" strokeWidth={3} />
                </button>
              </div>
            </>
          ) : role === "customer" ? (
